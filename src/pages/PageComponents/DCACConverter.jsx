...
import { defaultDCAConverterData } from "../../utils/defaultSavedFormat";
import deepEqual from "../../utils/objectComp";
import { toast } from "react-toastify";

function DCAConverter() {
  // Hooks
  ...
  // Input fields
  const [formData, setFormData] = useState(defaultDCAConverterData);
  const [originalFormData, setOriginalFormData] = useState(
    defaultDCAConverterData
  );
  ...
  useEffect(() => {
    ...
    const existingComponent = getComponent("dcAcConverter", currentComponentId);
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.description.trim()) {
      setError("Enter a description for the DC/AC converter");
      return;
    }
    ...
    try {
      if (isEditMode) {
        saveComponent("dcAcConverter", currentComponentId, cleanedData, {});
        toast.success("DC-AC-Converter Updated")
      } else {
        createComponent("dcAcConverter", cleanedData, {});
        toast.success("DC-AC-Converter Created")
      }
      navigate("/home");
    } catch (err) {
      setError(
        err.message || "Error saving DC/AC converter. Please try again."
      );
      toast.error("DC-AC-Converter Edit Failed")
    }
  };
  ...
  return (
    <div className="page">
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="form-actions">
            <button type="submit" className="submit-btn" disabled={!hasChanges}>
              {isEditMode ? "Update" : "Create"} DC/AC Converter
            </button>
            <button
              type="button"
      ...
      {/* Right Side - DC/AC Converter's own information display */}
      <div className="right">
        <h3>DC/AC Converter Information</h3>
